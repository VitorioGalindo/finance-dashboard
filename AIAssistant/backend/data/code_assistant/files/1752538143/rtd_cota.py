# RTD.py (Versão 7 - Adiciona Cálculo de Retorno Percentual)
...
def get_historical_data():
    """Endpoint para dados históricos. Usa pandas.read_excel e calcula o retorno."""
    try:
        ...
        if df_merged.empty:
            print("AVISO (Gráfico): O merge resultou em um dataframe vazio.")
            return jsonify([])

        # --- NOVO: Cálculo do retorno percentual ---
        # Pega o primeiro valor como base
        base_cota = df_merged['Cota'].iloc[0]
        base_ibov = df_merged['IBOV'].iloc[0]

        # Calcula o retorno em relação à base
        df_merged['cota_return'] = (df_merged['Cota'] / base_cota) - 1
        df_merged['ibov_return'] = (df_merged['IBOV'] / base_ibov) - 1
        
        # Formata a data para string no final para enviar via JSON
        df_merged['Data'] = df_merged['Data'].dt.strftime('%Y-%m-%d')
        
        historical_data = df_merged.to_dict(orient='records')
        print(f"Historical: Servidor enviou {len(historical_data)} registros para o gráfico, incluindo retornos.")
