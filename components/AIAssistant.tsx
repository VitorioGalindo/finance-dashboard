...
import { Chat, ChatMessage, Assistant } from '../types';
import { geminiService } from '../services/geminiService';
import { AI_ASSISTANTS, PlusIcon, TrashIcon, PaperclipIcon, SparklesIcon, PaperAirplaneIcon } from '../constants';

const AIAssistant: React.FC = () => {
  const [chats, setChats] = useState<Chat[]>([]);
  const [activeChatId, setActiveChatId] = useState<string | null>(null);
  const [activeAssistant, setActiveAssistant] = useState<Assistant>(AI_ASSISTANTS[0]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const stream = await geminiService.streamFinancialAnalysis(userMessage.content, activeAssistant.systemInstruction);
      ...
      <div className="w-80 bg-slate-800/50 border-r border-slate-700 flex flex-col p-2">
        ...
        <div className="border-t border-slate-700 pt-2 mt-2">
          {AI_ASSISTANTS.map(assistant => (
            <button key={assistant.id} onClick={() => setActiveAssistant(assistant)} className={`w-full text-left p-3 my-1 rounded-lg border-2 transition-colors ${activeAssistant.id === assistant.id ? 'bg-slate-700 border-sky-500' : 'bg-slate-800 border-slate-700 hover:border-slate-600'}`}>
            ...
            <div className="p-4 border-t border-slate-700">
              ...
              <form onSubmit={handleSendMessage} className="relative">
                <textarea
                  ...
                  placeholder={`Enviar uma mensagem para ${activeAssistant.name}... (Shift+Enter para nova linha)`}
